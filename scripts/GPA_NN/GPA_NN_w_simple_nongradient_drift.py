...
yaml_file = "../../configs/{dataset}-GPA_NN.yaml".format(dataset=p.dataset)#, generative_model=p.generative_model)
...
if param['L'] == None:
    param['expname'] = 'Regularized_%s_%s' % (param['exptype'], 'inf')
else:
    param['expname'] = 'Regularized_%s_%.4f' % (param['exptype'], param['L'])
if param['reverse'] == True:
    param['expname'] += '_reverse'
    
    
# Mixture/Regularized GPA settings ------------------------------------------------
if 'label' in param.keys():
    if len(param['label']) > 2:
        current_filename = __file__
        if 'nongradient' not in current_filename:
            ...
            dic_drifts = np.genfromtxt(f"../../data/{param['dataset']}_simple_avg_directions_dim{param['N_dim']}.csv", delimiter='\t')
            
            dic_drifts = dic_drifts[1:,1:]
            drifts = []
            for j, lab in enumerate(param['label'][:-1]):
                idxs = np.where((dic_drifts[:,0] == lab) & (dic_drifts[:,1] == param['label'][j+1]))
                drift_temp = dic_drifts[idxs, 2:]
                drifts.append(tf.constant(np.reshape(drift_temp, [1,-1]), dtype=tf.float32))
...
phi = model(NN_par)  # discriminator ML part
...
# ODE solver setting
from lib.transport_particles import calc_vectorfield, solve_ode, transport_w_non_gradient_drift
dPs = []
...
if param['termination_error_metric'] == 'W2':
    from lib.losses_torch import W2
...
t_idx = 1
max_t_idx = len(param['label'])
# intermediate targets to calculate error for termination
if param['dataset'] == 'Transport_genes':
    from scripts.util.generate_data import import_gene_expression_data_short
    X_intermediate, X_intermediate_label = import_gene_expression_data_short(N_dim=param['N_dim'], label=param['label'][t_idx], reduction=param['reduction'])
    
elif param['dataset'] == 'Test_transport_genes':
    from scripts.util.generate_data import import_test_gene_expression_data_short
    X_intermediate, X_intermediate_label = import_test_gene_expression_data_short(label=param['label'][t_idx])

intermediate_target = tf.constant(X_intermediate, dtype=tf.float32)

if 'label' in param.keys():
    if len(param['label']) > 2 and 'nongradient' in current_filename:
        drift_idx = 0
        drift = drifts[drift_idx]
        
        # time stepsize for nongradient vectorfield term
        iters_all_target = param['iter_transient_target'] + [param['epochs']]
        lr_NGD = 1/iters_all_target[drift_idx]
...
for it in range(1, param['epochs']+1): # Loop for updating particles P
    ...
    if 'label' in param.keys():
        if len(param['label']) > 2:
            if param['termination_error_metric'] == 'W2':
                N_W2_samples = min([500, P.shape[0],intermediate_target.shape[0]])

                subsample_idx_1 = np.random.choice(P.shape[0], size=N_W2_samples, replace=False)
                subsample_idx_2 = np.random.choice(intermediate_target.shape[0], size=N_W2_samples, replace=False)
                error = W2(P.numpy()[subsample_idx_1], intermediate_target.numpy()[subsample_idx_2])
            ...
            if (error < param['termination_error'] and t_idx < max_t_idx-1) or it in param['iter_transient_target']:
                ...
                if 'nongradient' not in current_filename:
                    ...
                    # time stepsize for nongradient vectorfield term
                    lr_NGD = 1/(iters_all_target[drift_idx] - iters_all_target[drift_idx-1])
        ...
        elif param['termination_error_metric'] == 'W2':
            N_W2_samples = min([500, P.shape[0],Q.shape[0]])
            subsample_idx_1 = np.random.choice(P.shape[0], size=N_W2_samples, replace=False)
            subsample_idx_2 = np.random.choice(Q.shape[0], size=N_W2_samples, replace=False)
            error = W2(P.numpy()[subsample_idx_1], Q.numpy()[subsample_idx_2])
            if error < param['termination_error']:
                break
    ...
    P = transport_w_non_gradient_drift(P, param['ratio_nongrad_2_grad']*lr_NGD, drift)
